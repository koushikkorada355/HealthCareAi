"""Mock EHR / Mock Healthcare System — replaceable via connector interface.
Supports failure injection (?fail=timeout) for the recovery demo."""
import time, uuid
...
def _auth(x_api_key: str | None):
    import os
    if (x_api_key or "") != os.getenv("MOCK_EHR_API_KEY", API_KEY): raise HTTPException(401, "bad ehr key")

class ApptIn(BaseModel):
    patient_ref: str = ""; patient_name: str = ""; provider_id: str = ""; facility_id: str = ""
    starts_at: str = ""; ends_at: str = ""; reason: str = ""; idempotency_key: str = ""

@app.get("/health")
def health(): return {"ok": True, "service": "mock-ehr", "appointments": len(store["appointments"])}
...
def create(a: ApptIn, request: Request, x_api_key: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
    ...
    if key and key in store["by_idem"]: return store["by_idem"][key]  # idempotent replay
    ...
    if key: store["by_idem"][key] = rec
    return rec

@app.get("/ehr/appointments/{eid}")
def get_one(eid: str, x_api_key: str | None = Header(default=None)):
    _auth(x_api_key)
    if eid not in store["appointments"]: raise HTTPException(404, "not found")
