"""MCP adapter: schema validation + routing into the existing registry.

The adapter knows every attached tool's contract (required args, int-like
fields) and rejects bad calls WITHOUT invoking anything. Valid calls go
through mcp/client.py (timeout + normalized results + audit via registry).

No business logic lives here — only contracts. The registry + services
remain the sole implementers.
"""
from __future__ import annotations

from . import client as mcp_client

# Tool → required arg names. Everything else passes through untouched.
REQUIRED_ARGS: dict[str, list[str]] = {
    ...
    "update_preferences": [],
    "verify_external_appointment": ["external_id"],
...
# Args that must coerce to int when present and non-empty.
INT_ARGS = {
    "check_availability": ["doctor_id"],
    "get_appointment": ["appointment_id"],
    ...
    "get_questionnaire": ["questionnaire_id"],
    "submit_questionnaire": ["response_id"],
    "synchronize_state": ["appointment_id", "operation_id"],
    "get_context": ["conversation_id"],
}
...
def validate(name: str, args: dict | None) -> tuple[dict, str]:
    """Returns (coerced_args, error). Empty error means valid. Never calls."""
    from app.mcp import registry as reg
...
async def call(db, name: str, args: dict | None, *, user, conversation_id=None,
    ...
    """Validate then invoke. Always returns client.invoke's normalized dict."""
    coerced, error = validate(name, args)
    if error:
        return {"ok": False, "code": "invalid", "error": error,
                "latency_ms": 0, "tool_call_id": tool_call_id}
    out = await mcp_client.invoke(db, name, coerced, user=user,
